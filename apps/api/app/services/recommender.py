...
def extract_features(phone: PhoneDetails, persona_idx: int, budget: float) -> list:
    ...
    if '7000' in raw_html or '7300' in raw_html: battery = 7000
    elif '6000' in raw_html: battery = 6000
    ...
    if any(k in raw_html for k in ['snapdragon 8 elite', 'dimensity 9400', 'a18', 'a17']): perf = 4
    elif any(k in raw_html for k in ['snapdragon 8 gen 3', 'dimensity 9300', 'a16']): perf = 3
    elif any(k in raw_html for k in ['snapdragon 7', 'dimensity 8']): perf = 2
    elif any(k in raw_html for k in ['snapdragon 6', 'dimensity 7']): perf = 1
...
def persona_name_to_idx(name: str) -> int:
    ...
    if 'camera' in name or 'creator' in name or 'photo' in name: return 2
    if 'pro' in name or 'business' in name: return 3
    return 4
...
def ml_score_phones(phones: List[PhoneDetails], persona: str, budget: float, semantic_ids: List[str] = None) -> List[Dict[str, Any]]:
    ...
    for i, phone in enumerate(valid_phones):
        ...
        if is_creator and sentiment.get("camera", 0) > 0.2:
            score += 6.0
            reasons.append("Community Acclaim: Top-rated photo and video dynamic range in reviewer tests")
        elif is_gamer and sentiment.get("performance", 0) > 0.2:
            score += 6.0
            reasons.append("Benchmark Verified: Low thermal throttling in sustained gaming stress tests")
        elif is_student and sentiment.get("battery", 0) > 0.2:
            score += 6.0
            reasons.append("Real-World Endurance: Praised screen-on time in reviewer endurance tests")
