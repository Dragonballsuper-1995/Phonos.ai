...
================================
Generates high-resolution, publication-grade System Architecture Diagram matching
the clean black-and-white box-and-arrow aesthetic of VulnLoc System Architecture.
"""
import os
...
def generate_diagram(output_path: str):
    ...
    def draw_box(x, y, w, h, title, subtitle=''):
        ...
        ax.add_patch(bx)
        if subtitle:
            ax.text(x, y + h*0.15, title, ha='center', va='center', fontsize=11, fontweight='bold', color='#000000', family='sans-serif')
            ax.text(x, y - h*0.20, subtitle, ha='center', va='center', fontsize=8.8, color='#111111', family='sans-serif')
        else:
            ax.text(x, y, title, ha='center', va='center', fontsize=10.8, fontweight='bold', color='#000000', family='sans-serif')

    def draw_straight_arrow(x1, y1, x2, y2):
        ax.annotate(
            '', xy=(x2, y2), xytext=(x1, y1),
            arrowprops=dict(facecolor='#000000', edgecolor='#000000', arrowstyle='->,head_width=0.35,head_length=0.35', lw=1.5)
        )
    ...
    def draw_elbow_arrow(points):
        for i in range(len(points) - 1):
            ...
            if i == len(points) - 2:
                ...
                ax.plot([x1, x2], [y1, y2], color='#000000', lw=1.5)

    # 1. Top Input
    draw_box(50, 93, 38, 5.5, 'User Query & Intent Inputs', 'Persona  |  Budget (₹)  |  5D Priority Sliders  |  Natural Intent')
    ...
    draw_box(50, 32.5, 62, 6.2, 'Stage 2: 2-Stage DLRM Ranking Engine', 'Pre-Trained XGBoost (ranker.xgb)  +  25-Pt Bonus Cap  +  Brand Diversity Filter')
    ...
    draw_box(50, 7.5, 72, 6.2, 'Presentation & Closed-Loop RLHF Feedback', 'Next.js Typographic Accordion & Deep Report  <--->  PostHog Telemetry & Retrainer')
